...
| 6 | `evaluate.py` + `compare_runs.py` + write-up | `docs/phase-6-evaluation-comparison.md` | **Done** — both checkpoints evaluated on the full 7,404-row held-out set, 4 comparison plots in `results/`; verdict is inconclusive at this scale (3 of 4 "more training needed?" criteria trigger — held-out reverses the training-time direction, the gap is within single-run noise, and `outcome_only`'s tool-call frequency rose rather than fell as the paper claims); see phase doc's Handoff notes for full numbers and the recommended symmetric re-run (both conditions, same larger step budget + new seed — not yet launched, a real GPU-hours decision left to the user) |
