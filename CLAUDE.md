...
| 6 | `evaluate.py` + `compare_runs.py` + write-up | `docs/phase-6-evaluation-comparison.md` | **Done, resolved positive** — original run (seed42/300steps) was inconclusive (3 of 4 "more training needed?" criteria triggered); the recommended symmetric re-run (seed123/600steps, both conditions, same larger budget + new seed) resolved it: `turn_level` now shows a real, held-out-confirmed advantage over `outcome_only` (EM 0.307 vs 0.242, F1 0.399 vs 0.343), directionally consistent with training for the first time, and `retrieval_fraction` rose instead of declining (0.40→0.57). Only criterion 3 still triggers (`outcome_only`'s tool-call frequency still rises instead of falling, contradicting the paper's claimed mechanism) — an independent finding, not part of the main comparison. Two follow-up reward-design experiments (`length_penalty`, `search_count_penalty`) in progress to address the completion-length drift and the tool-call-frequency mismatch; see phase doc's Handoff notes for full numbers, the `beta=0`-matches-the-paper correction, and both follow-ups' honest framing (neither is a strict paper-fidelity fix) |
