...
| 4 | `train.py` + live smoke test | `docs/phase-4-training-smoke-test.md` | **In progress** — `train.py`/`scripts/verify_phase4.py` implemented and reviewed (`build_config`, `TrackioAlertCallback`, CLI, `build_trainer`/`main`), `verify_phase4.py` passes; the live GPU smoke test (both conditions, manual transcript read, trackio alert check) is the only remaining exit criterion — see the design spec's "Smoke test execution" section |
